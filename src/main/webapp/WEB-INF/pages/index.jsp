...
<html>
	...
	<body onload="autoUpdatePendingGames()">
		<table id="menu" width=100%>
			<tr>
				<td width="100%" style="display: ${statistic_visibility}"> <st:message code="hello" />, ${username}. 
			...
			<tr>
				<td align="left" width="90%"><a id="registration" href="registration.html" style="display:${registration_visibility}"><st:message code="registration" /> |</a>
					...
					<a id="creategame" href="" style="display:${create_game_visibility}" onclick="clickPopUp()"><st:message code="creategame" /> |</a>
		...
		<div id="popup" style="display:none">
			...
			<input type="radio" name="color" id="black" value="black"><label for="black"><st:message code="black" /></label>
			<input type="radio" name="color" id="random" value="random"><label for="random"><st:message code="random" /></label></p>
		...
		<table id="games" width=60% border="1" align="center" style="display:${pending_games_visibility}">
			<c:forEach items="${pendingGames}" var="pendingGame">
				<tr>
					<c:if test="${not empty pendingGame.whiteId}">
					    <td width="20%">${pendingGame.whiteId.username}</td>
					    <td width="20%"><st:message code="black" /></td>
					</c:if>
					<c:if test="${not empty pendingGame.blackId}">
					    <td width="20%">${pendingGame.blackId.username}</td>
